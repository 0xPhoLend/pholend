...
export const calcExpectedUserDataAfterRepay = (
	...
	if (rateMode == RateMode.Stable) {
		...
		expectedUserData.principalStableDebt =
			expectedUserData.currentStableDebt = stableDebt.minus(totalRepaidBN)
...
export const calcExpectedUserDataAfterSwapRateMode = (
	...
	if (rateMode === RateMode.Stable) {
		...
		expectedUserData.currentVariableDebt =
			expectedUserData.scaledVariableDebt = new BigNumber(0)
	}
...
export const calcExpectedInterestRates = (
	...
	const asset = networksData.test.find(
	...
	const reserveConfiguration = asset.config
	...
	if (utilizationRate.gt(optimalRate)) {
		...
		stableBorrowRate = stableBorrowRate
			...
			.plus(
				new BigNumber(
					reserveConfiguration.strategy.stableRateSlope2,
		...
		variableBorrowRate = variableBorrowRate
			...
			.plus(
				new BigNumber(
					reserveConfiguration.strategy.variableRateSlope2,
				).rayMul(excessUtilizationRateRatio),
			)
	} else {
		stableBorrowRate = stableBorrowRate.plus(
			new BigNumber(
				reserveConfiguration.strategy.stableRateSlope1,
			).rayMul(utilizationRate.rayDiv(new BigNumber(optimalRate))),
		)
		...
		variableBorrowRate = variableBorrowRate.plus(
			utilizationRate
				...
				.rayMul(
					new BigNumber(
						reserveConfiguration.strategy.variableRateSlope1,
					),
				),
		)
	}
	...
	const liquidityRate = expectedOverallRate
		...
		.percentMul(
			new BigNumber(PERCENTAGE_FACTOR).minus(
				reserveConfiguration.reserveFactor,
			),
		)
